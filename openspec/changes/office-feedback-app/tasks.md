...
- [ ] 1.7 Convert `docs/desk-allocation/Desk_Allocation_Anonymized.xlsx` (per `AI_AGENT_GUIDE.md`) into the committed/bundled app asset used by repo, CI, and demo builds; if local-only internal builds ever support the real `Desk_Allocation.xlsx`, keep both the input path and any generated output ignored/untracked so real employee data is never committed or bundled into repo-visible assets
...
- [ ] 2.6 Build the remaining onboarding steps around the selected employee as the source of truth: pre-fill desk area from the employee record, auto-derive building/floor/zone from that desk assignment, and allow manual desk entry only when the selected employee has no assigned desk in the dataset
