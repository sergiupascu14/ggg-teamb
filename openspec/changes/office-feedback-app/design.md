...
- **Desk allocation dataset as canonical location reference.** The anonymized workbook and `AI_AGENT_GUIDE.md` in `docs/desk-allocation/` are the source of truth for the desk ID grammar (`{Building}{Floor}-{Zone}{Row}-{DeskNum}`), the building/floor enumeration (Tower 3–6, Riviera 3–5), zones A–D, and Staff ID → name/supervisor/desk mappings. At build time the relevant sheets are converted to a bundled asset (e.g. JSON in `app/src/main/assets`) and loaded into a local lookup table. Onboarding validates the entered desk area against this grammar and can auto-derive building/floor/zone from it; identity can be resolved from Staff ID locally as a stand-in/complement for GarminAD. The newsfeed building/floor filters draw their options from the same enumeration. Alternative (hardcoding building/floor lists in the UI) rejected to avoid drift from the real layout.
